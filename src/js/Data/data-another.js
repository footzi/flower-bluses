let products = [{
    "id":"5-1",
    "name":"Браслет «Лесная поляна»",
    "cost":"под заказ",
    "text":"Этот браслет из ягод черники, брусники, черной смородины и облипихи обязательно будет напоминать вам вкус лета даже самой морозной зимой. Каждая ягодка вылеплена и тонирована вручную.",
    "imagePreview":"/images/catalog/another/preview/браслет 1.jpg",
    "image1":"/images/catalog/another/браслет 1.png",
    "image2":"/images/catalog/another/браслет 1.1.png",
},{
    "id":"5-2",
    "name":"Браслет «Черничная полянка»",
    "cost":"под заказ",
    "text":"Браслет с ягодкам черники и цветами ромашки обязательно будет дарить вам летнее настроение круглый год. Ручная работа.",
    "imagePreview":"/images/catalog/another/preview/браслет 2.jpg",
    "image1":"/images/catalog/another/браслет 2.png",
    "image2":"/images/catalog/another/браслет 2.1.png",
},{
    "id":"5-3",
    "name":"Ободок «Вкус лета»",
    "cost":"под заказ",
    "text":"Ободок с ягодками , листочками и даже цветочками черники. Он обязательно будет дарить вам летнее настроение круглый год. Ручная работа.",
    "imagePreview":"/images/catalog/another/preview/обруч 1.jpg",
    "image1":"/images/catalog/another/обруч 1.3.png",
    "image2":"/images/catalog/another/обруч 1.1.png",
    "image3":"/images/catalog/another/обруч 1.2.png",
    "image4":"/images/catalog/another/обруч 1.png",
},{
    "id":"5-4",
    "name":"",
    "cost":"",
    "text":"",
    "imagePreview":"/images/catalog/another/preview/невидимки 1.jpg",
    "image1":"/images/catalog/another/невидимки 1.png",
    "image2":"/images/catalog/another/невидимки 1.1.png",
    "image3":"/images/catalog/another/невидимки 1.2.png"
}];
export  { products };