let products = [{
    "id":"4-1",
    "name":"Брошь «Нежность сирени»",
    "cost":"под заказ",
    "text":"Нежная брошь с веточкой сирени. Каждый цветок вылеплен и тонирован вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/брошь 1.png",
    "image1":"/images/catalog/сlips/брошь 1.png",
    "image2":"/images/catalog/сlips/брошь 1.1.png",
},{
    "id":"4-2",
    "name":"Брошь «Веточка жасмина»",
    "cost":"под заказ",
    "text":"Нежная брошь-бутоньерка с веточкой жасмина. Каждый цветок выполнен вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/брошь 2.jpg",
    "image1":"/images/catalog/сlips/брошь 2.png",
    "image2":"/images/catalog/сlips/брошь 2.1.png",
},{
    "id":"4-3",
    "name":"Брошь «Запах весны»",
    "cost":"под заказ",
    "text":"Красивая и нежная брошь-бутоньерка. Каждый цветок вылеплен и тонирован вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/брошь 3.jpg",
    "image1":"/images/catalog/сlips/брошь 3.png",
    "image2":"/images/catalog/сlips/брошь 3.1.png",
    "image3":"/images/catalog/сlips/брошь 3.2.png",
},{
    "id":"4-4",
    "name":"Брошь «Вкус осени»",
    "cost":"под заказ",
    "text":"Стильная, яркая брошь как напоминание о жаркой осени. Каждый цветок, каждая ягодка выполнены и тонированы вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/брошь 4.jpg",
    "image1":"/images/catalog/сlips/брошь 4.png",
    "image2":"/images/catalog/сlips/брошь 4.1.png",
},{
    "id":"4-5",
    "name":"Брошь «Сиреневый май»",
    "cost":"под заказ",
    "text":"Брошка с нежной веточкой сирени. Каждый цветок выполнен вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/брошь 5.jpg",
    "image1":"/images/catalog/сlips/брошь 5.png",
    "image2":"/images/catalog/сlips/брошь 5.1.png",
    "image3":"/images/catalog/сlips/брошь 5.2.png",
},{
    "id":"4-6",
    "name":"Заколка «Снежная сирень»",
    "cost":"под заказ",
    "text":"Зажим для волос с нежной сиренью. Веточка натурального размера. Каждый цветочек вылеплен и тонирован вручную. Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 1.jpg",
    "image1":"/images/catalog/сlips/заколка 1.png",
    "image2":"/images/catalog/сlips/заколка 1.1.png",
},{
    "id":"4-7",
    "name":"Заколка «Кусочек весны»",
    "cost":"под заказ",
    "text":"Нежная заколка с цветами сирени. Каждый цветочек вылеплен и тонирован вручную. Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 2.jpg",
    "image1":"/images/catalog/сlips/заколка 2.png",
},{
    "id":"4-8",
    "name":"Заколка «Весенний букет»",
    "cost":"под заказ",
    "text":"Заколка с нежными цветами синей незабудки, белыми колокольчиками и сиренью. Каждый цветок вылеплен и тонирован вручную. Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 3.jpg",
    "image1":"/images/catalog/сlips/заколка 3.png",
    "image2":"/images/catalog/сlips/заколка 3.1.png",
    "image3":"/images/catalog/сlips/заколка 3.2.png",
    "image4":"/images/catalog/сlips/заколка 3.3.png",
},{
    "id":"4-9",
    "name":"Заколка «Веточка малины»",
    "cost":"под заказ",
    "text":"Заколка с веточкой малины будет радовать вас летним настроением даже в самый холодный зимний день. Ручная работа.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 4.jpg",
    "image1":"/images/catalog/сlips/заколка 4.png",
},{
    "id":"4-10",
    "name":"Брошь «Весенняя с незабудками»",
    "cost":"1600 руб.",
    "text":"Брошка-бутоньерка с цветами голубой незабудки и ландыша обязательно напомнит Вам чудесные мгновения весны! Каждый цветок вылеплен и тонирован вручную.  Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 5.jpg",
    "image1":"/images/catalog/сlips/заколка 5.jpg",
    "image2":"/images/catalog/сlips/заколка 5.1.png",
    "image3":"/images/catalog/сlips/заколка 5.2.png",
},{
    "id":"4-11",
    "name":"Заколка «Ягодный микс»", 
    "cost":"под заказ",
    "text":"Заколка с ягодками черники, черной смородины и брусники. Ручная работа. Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 6.jpg",
    "image1":"/images/catalog/сlips/заколка 6.png",
    "image2":"/images/catalog/сlips/заколка 6.1.png",
    "image3":"/images/catalog/сlips/заколка 6.2.png",
},{
    "id":"4-12",
    "name":"Заколка «Полевые цветы»",
    "cost":"1100 руб.",
    "text":"Заколка с цветами ромашки, ландышем и ягодками черники. Ручная работа. Возможно изготовление броши.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 7.jpg",
    "image1":"/images/catalog/сlips/заколка 7.png",
    "image2":"/images/catalog/сlips/заколка 7.1.png",
    "image3":"/images/catalog/сlips/заколка 7.2.png",
},{
    "id":"4-13",
    "name":"Брошь «Бархат»",
    "cost":"1200 руб.",
    "text":"Нежная брошь с цветами сирени и яблоневым цветом. Каждый цветок выполнен и тонирован вручную. Возможно изготовление заколки.",
    "imagePreview":"/images/catalog/сlips/preview/заколка 8.jpg",
    "image1":"/images/catalog/сlips/заколка 8.2.png",
    "image2":"/images/catalog/сlips/заколка 8.1.png",
    "image3":"/images/catalog/сlips/заколка 8.png",
}];
export  { products };